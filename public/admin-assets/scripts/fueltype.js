        ...
        submitHandler: function (form) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.success) {
                        $("#fuelTypeTable").DataTable().ajax.reload(null, false);
...
$(document).on("click", ".edit-fuelType", function () {
    ...
    $.ajax({
        url: baseUrl + "/fuel-types/" + encryptedId + "/edit",
        type: "GET",
        beforeSend: function () {
            $("#loader").show();
        },
        success: function (res) {
            ...
            if (res.status) {
                ...
                $("#fuelTypeModal #editId").val(res.doc.id);
                $("#fuelTypeModal #Name").val(res.doc.name);
                // Display brand image if exists
                if (res.doc.image) {
                    // check if brandImg is not null/empty
                    const signUrl = baseUrl + "/storage/" + res.doc.image;
                    $("#showImg").html(
                        `<img src="${signUrl}" alt="fuelTypeImg" style="max-width: 100px; height: 60px;">`
                    );
                } else {
...
$(document).on("click", ".view-fuelType", function () {
    let encryptedId = $(this).data("id");
    let baseUrl = $('meta[name="base-url"]').attr("content");
    $.ajax({
        url: baseUrl + "/fuel-types/" + encryptedId + "/edit",
